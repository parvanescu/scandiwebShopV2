...
class CartItemButtons extends Component{
    render() {
        ...
        return (
            <ButtonsWrapper>
                <CartButton>
                    <PlusMinusIcon isPlus={true} onClickCallback={()=>this.props.changeQuantity(product.id,this.props.getQuantity(product.id)+1)}/>
                </CartButton>
                <QuantityDisplay>
                    {this.props.getQuantity(this.props.product.id)}
                </QuantityDisplay>
                <CartButton>
                    <PlusMinusIcon onClickCallback={()=>this.props.changeQuantity(product.id,this.props.getQuantity(product.id)-1)}/>
                </CartButton>
            </ButtonsWrapper>
        );
    }
